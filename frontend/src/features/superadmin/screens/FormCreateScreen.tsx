...
export default function FormCreateScreen() {
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.screenId) {
      Alert.alert("Error", "Please select a screen");
      return;
    }
    ...
    setLoading(true);
    try {
      // Convert fields.required to string 'true' or 'false' for backend validation
      ...
      if (response.success) {
        ...
        Alert.alert("Error", response.message || "Failed to create form");
      }
    } catch (error) {
