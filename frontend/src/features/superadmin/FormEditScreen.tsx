...
export const FormEditScreen: FC<FormEditScreenProps> = ({}) => {
  ...
  const handleUpdateForm = async () => {
    ...
    if (!form) {
      showToast({ type: "error", message: "No form data" });
      return;
    }
    ...
    try {
      ...
      if (response.success) {
        showToast({ type: "success", message: "Form updated successfully" });
        router.back();
      } else {
        showToast({
          type: "error",
          message: response.message || "Failed to update form",
        });
      }
    } catch (error) {
      console.error("Error updating form:", error);
      showToast({ type: "error", message: "Failed to update form" });
    } finally {
      setLoading(false);
    }
  };
          ...
          onPress: async () => {
            ...
            try {
              ...
              if (response.success) {
                ...
                showToast({
                  type: "error",
                  message: response.message || "Failed to delete form",
                });
              }
            } catch (error) {
              console.error("Error deleting form:", error);
              showToast({ type: "error", message: "Failed to delete form" });
            } finally {
              setLoading(false);
            }
          },
