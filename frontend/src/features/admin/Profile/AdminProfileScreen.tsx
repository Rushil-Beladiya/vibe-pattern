...
export const AdminProfileScreen: FC = () => {
  const params = useLocalSearchParams();
  const screen_id =
    (params.screen_id as string) || (params.id as string) || "3";
  ...
  const fetchScreenData = async () => {
    try {
      ...
      const response = await sendRequest({
        url: `admin/screens/${screen_id}`,
        method: "get",
      });

      if (response.success && response.data) {
        // Normalize forms/fields to arrays (API may return JSON strings)
        ...
        const normalized = { ...raw, forms };

        setScreenData(normalized);
        if (forms.length > 0) {
      ...
      console.warn("AdminProfile fetch error:", err);
